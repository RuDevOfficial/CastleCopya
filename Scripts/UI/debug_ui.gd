...
func _ready() -> void:
	if (mute_music == true): AudioManager.overwrite_volume(0)
...
func _on_toggle_music_pressed() -> void:
	if (mute_music == true): AudioManager.overwrite_volume(1)
	else: AudioManager.overwrite_volume(0)
	
	mute_music = !mute_music

func _on_super_speed_pressed() -> void:
	if (speed_up_character == false): 
		player_character.player_resource.Speed *= player_speed_multiplier
	else:
		player_character.player_resource.Speed /= player_speed_multiplier
...
func _on_super_jump_pressed() -> void:
	if (increase_jump_character == false): 
		player_character.player_resource.JumpForce *= increase_jump_multiplier
	else:
		player_character.player_resource.JumpForce /= increase_jump_multiplier
