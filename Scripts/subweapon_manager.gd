...
func try_use_subweapon(direction : int, position : Vector2) -> void:
	if (can_use_subweapon()): use_subweapon(direction, position)
...
func use_subweapon(direction : int, position : Vector2) -> void:
	...
	subweapon_instance.global_position = position
